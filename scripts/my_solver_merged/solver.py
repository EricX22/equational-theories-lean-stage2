...
# Master switch for ALL LLM stages (waypoints, direct proof, strategist,
# fallback). On by default; set False for a clean deterministic-only run — the
...
def main():
    ...
    # LLM master switch: skip every LLM stage when disabled (deterministic-only
    # ablation). Placed after all deterministic stages, so turning it off leaves
