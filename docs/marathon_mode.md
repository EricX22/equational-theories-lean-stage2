...
| `examples/marathon/demos/triage_oss/`   | Difficulty-sorted Pass B + budget-aware Pass C refinement on Pass-B failures (entry-level LLM).  |
| `examples/marathon/demos/fewshot_oss/`  | In-run lemma cache + few-shot transfer (cross-problem state — Marathon-only strategy).           |
